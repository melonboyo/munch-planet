...
func process(delta) -> void:
	...
	_physics_rot *= Quaternion(Vector3.FORWARD, _rot_momentum * delta)
	_physics_rot = _physics_rot.normalized()
...
func update_physics_object(delay : float) -> void:
	...
	_physics_rot = _current_physics_object().global_transform.basis.get_rotation_quaternion().normalized()
