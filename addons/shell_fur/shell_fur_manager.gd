...
# For static meshes it use a MultiMeshInstance3D for skinned meshes it will create
...
var custom_shader : Shader:
	set(shader):
		...
		if shader != null:
			shader_type = SHADER_TYPES.CUSTOM
		else:
			shader_type = SHADER_TYPES.REGULAR
...
var layers := 40: 
	set(new_layers):
		...
		_update_fur(0.0)
var pattern_selector := 0: 
	set(index): 
		pattern_selector = index
		if _first_enter_tree:
			return
		if index != PATTERNS.size():
			pattern_texture = load(PATTERNS[index])
		else:
			set_shader_param("i_pattern_texture", pattern_texture)
		notify_property_list_changed()
var pattern_texture : Texture:
	set(texture): 
		...
		if is_custom:
			pattern_selector = PATTERNS.size()
var pattern_uv_scale = 5.0:
	set(value): 
...
# Physics
var physics_custom_physics_pivot : NodePath: 
	set(path): 
...
func _update_fur(delay : float) -> void:
	...
	if _parent_has_skin_assigned:
		...
		FurHelperMethods.generate_mmi(layers, _multimeshInstance, _parent_object.mesh, _material, styling_blendshape - 1, cast_shadow, _parent_object.global_position)
